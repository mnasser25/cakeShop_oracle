...
if(filter_input(INPUT_GET, 'action') == 'delete'){
    ...
    $Q_delete_cartitem = 'DELETE FROM ORDER_ WHERE CAKE_ID = '.filter_input(INPUT_GET, 'cake_id');
...
$Q_fetch_featured = "SELECT * FROM ORDER_ INNER JOIN CAKES ON CAKES.CAKE_ID = ORDER_.CAKE_ID "; //selects featured products
$Q_fetch__all_products = "SELECT * FROM ORDER_";
...
<div class="row mx-auto">
    ...
    <div class="col-lg">
        ...
        foreach($_SESSION['shopping_cart'] as $key => $product){
            ...
            <div class="receipt-card mt-2 mb-3 mx-1 py-3">
                ...
                $result_product = oci_parse($conn, $Q_fetch__all_products);
                $check = oci_num_rows($result_product);

                if($check>0){ //checks if $result empty in database
                    //loops through all items in products table in database
                    while($product_row = oci_fetch_assoc($result_product)){
    ...
    if(isset($_SESSION['shopping_cart'])) {
        if(count($_SESSION['shopping_cart']) == 0) {
            ...
            echo('<div class="text-center py-3"><a href="add.php" class="button button__round">SHOP NOW</a></div>');
